...
export default function ProjectCard({ project }: any) {
  return (
    <motion.article 
      ...
      className="rounded-xl overflow-hidden bg-neutral-900/40 border border-white/5"
    >
      <Link to={`/projects/${project.slug}`}>
        <div className="relative aspect-video overflow-hidden">
          ...
          {project.instagramUrl && (
            <div className="absolute top-3 right-3 px-2 py-1 bg-black/60 rounded-full flex items-center gap-1">
              <svg className="w-3 h-3 text-[#ff1493]" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-4">
          <span className="text-[#ff1493] text-xs uppercase tracking-wider">{project.category}</span>
